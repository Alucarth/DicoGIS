...
class TabFiles(Frame):
    def __init__(self, parent, txt=dict(), path_browser=None, path_var=""):
        ...
        caz_pdf = Checkbutton(
            self.FrFilters, text="Geospatial PDF", variable=self.opt_pdf
        )
        self.caz_lyr = Checkbutton(self.FrFilters, text=".lyr", variable=self.opt_lyr)
        self.caz_mxd = Checkbutton(self.FrFilters, text=".mxd", variable=self.opt_mxd)
        caz_qgs = Checkbutton(self.FrFilters, text=".qgs", variable=self.opt_qgs)
        # widgets placement
        ...
        caz_pdf.grid(row=1, column=5, columnspan=2, sticky="NSWE", padx=2, pady=2)
        ...
        self.caz_lyr.grid(row=3, column=0, columnspan=2, sticky="NSWE", padx=2, pady=2)
        self.caz_mxd.grid(row=3, column=1, columnspan=2, sticky="NSWE", padx=2, pady=2)
        caz_qgs.grid(row=3, column=2, columnspan=2, sticky="NSWE", padx=2, pady=2)
    ...
    def get_target_path(self, def_rep):
        ...
        foldername = askdirectory(
            parent=self.p, initialdir=def_rep, mustexist=True, title=""
        )
        ...
        self.output.insert(
            0, "DicoGIS_{0}_{1}.xlsx".format(path.split(self.target_path.get()), today)
        )
