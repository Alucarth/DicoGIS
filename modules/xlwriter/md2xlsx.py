...
class md2xlsx(Workbook):
    ...
    def set_worksheets(
        ...
        if has_lyr and "LYR" not in self.sheetnames:
            ...
            for i in self.li_cols_lyr:
                self.ws_lyr.cell(
                    row=1, column=self.li_cols_lyr.index(i) + 1
                ).style = "Headline 2"
            # initialize line counter
            self.idx_l = 1
        else:
    ...
    def store_md_vector(self, layer):
        ...
        self.ws_v.cell("N{}".format(self.idx_v)).value = " |\n ".join(
    ...
    def store_md_raster(self, layer, bands):
        ...
        self.ws_r.cell("S{}".format(self.idx_v)).value = " |\n ".join(
            layer.get("dependencies")
        )
        ...
        for (layer_idx, layer_name) in zip(
            ...
            try:
                ...
                gdb_layer = filedb.get(
                    "{0}_{1}".format(layer_idx, unicode(layer_name.decode("latin1")))
                )
            # in case of a source error
            ...
            # Spatial extent
            emprise = "Xmin : {0} - Xmax : {1} | \nYmin : {2} - Ymax : {3}".format(
                unicode(gdb_layer.get("Xmin")),
                unicode(gdb_layer.get("Xmax")),
                unicode(gdb_layer.get("Ymin")),
                unicode(gdb_layer.get("Ymax")),
            )
