...
class OptionsManager(object):
    ...
    def save_settings(self, parent):
        ...
        self.config.set(
            "filters", "opt_spadb", unicode(parent.tab_files.opt_spadb.get())
        )
        ...
        self.config.set(
            "database", "opt_views", unicode(parent.tab_sgbd.opt_pgvw.get())
        )
        # proxy settings
        self.config.set(
            "proxy", "proxy_needed", unicode(parent.tab_options.opt_proxy.get())
        )
        self.config.set(
            "proxy", "proxy_type", unicode(parent.tab_options.opt_ntlm.get())
        )
        self.config.set("proxy", "proxy_server", parent.tab_options.prox_server.get())
        self.config.set(
            "proxy", "proxy_port", unicode(parent.tab_options.prox_port.get())
        )
        self.config.set("proxy", "proxy_user", parent.tab_options.prox_user.get())
        # Isogeo settings
        self.config.set(
            "isogeo", "opt_isogeo", unicode(parent.tab_options.opt_isogeo.get())
        )
