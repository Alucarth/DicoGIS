        ...
        if srs:
            ...
            try:
                if srs.GetAttrValue("PROJCS") != "unnamed":
                    dico_layer["srs"] = unicode(srs.GetAttrValue("PROJCS")).replace(
                        "_", " "
                    )
                else:
                    dico_layer["srs"] = unicode(srs.GetAttrValue("PROJECTION")).replace(
                        "_", " "
                    )
                ...
                dico_layer["EPSG"] = unicode(srs.GetAttrValue("AUTHORITY", 1))
...
if __name__ == "__main__":
    ...
    for dxfpath in li_dxf:
        ...
        if path.isfile(dxfpath):
            print("\n{0}: ".format(dxfpath))
            Read_DWG(dxfpath, dico_dxf, "AutoCAD DXF", textos)
            # print results
            print(dico_dxf)
        else:
            pass
