...
class ReadMXD:
    def __init__(self, mxd_path, dico_mxd, tipo, txt=""):
        """ Uses arcpy functions to extract basic informations about
        ...
        dico_mxd["total_size"] = self.sizeof(size)

        # global dates
        dico_mxd["date_crea"] = strftime("%d/%m/%Y", localtime(path.getctime(mxd_path)))
    ...
    def infos_dataframe(self, dataframe, dico_dframe):
        ...
        dico_dframe["Xmin"] = round(extent.XMin, 2)
        dico_dframe["Xmax"] = round(extent.XMax, 2)
        dico_dframe["Ymin"] = round(extent.YMin, 2)
        dico_dframe["Ymax"] = round(extent.YMax, 2)

        # map settings
        dico_dframe["mapUnits"] = dataframe.mapUnits

        # SRS
        srs = extent.spatialReference
        dico_dframe["srs"] = srs.name
        dico_dframe["srs_type"] = srs.type
        if srs.type == "Projected":
            ...
            dico_dframe["EPSG"] = srs.GCSCode, srs.GCSName, srs.datumCode, srs.datumName
        else:
            dico_dframe["EPSG"] = (
    ...
    def erratum(self, dico_mxd, mxd_path, mess):
        ...
        dico_mxd["name"] = path.basename(mxdpath)
        dico_mxd["folder"] = path.dirname(mxdpath)
        dico_mxd["error"] = mess
        # End of function
        return dico_mxd
...
if __name__ == "__main__" and __package__ is None:
    ...
    li_mxd = [
        path.abspath(mxd) for mxd in li_mxd if path.splitext(mxd)[1].lower() == ".mxd"
    ]
    ...
    for mxdpath in li_mxd:
        ...
        if path.isfile(mxdpath):
            print("\n{0}: ".format(mxdpath))
            ReadMXD(mxdpath, dico_mxd, "Esri MXD", txt=textos)
            # print results
            print(dico_mxd)
        else:
