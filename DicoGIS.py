...
class DicoGIS(Tk):
    ...
    def __init__(self):
        ...
        self.title("DicoGIS {0}".format(self.DGversion))
        if opersys == "win32":
        ...
        # fillfulling text
        TextsManager().load_texts(
            dico_texts=self.blabla, lang=self.def_lang, locale_folder=r"data/locale"
        )
        ...
        # =================================================================================
        # ## TAB 3: web services ##
        self.nb.add(self.tab_webservices, text=" Web Service ", padding=3)

        # =================================================================================

        # ## TAB 4: Isogeo ##
        self.nb.add(self.tab_isogeo, text="Isogeo", padding=3)
        ...
        # widgets
        self.nameoutput = Label(self.FrOutp, text=self.blabla.get("gui_fic"))
        self.output = Entry(self.FrOutp, width=35)
        # widgets placement
        self.nameoutput.grid(row=0, column=1, sticky="NSWE", padx=2, pady=2)
        self.output.grid(row=0, column=2, columnspan=1, sticky="NSWE", padx=2, pady=2)
        ...
        utils_global.ui_switch(
            self.tab_options.opt_isogeo, self.tab_options.FrOptIsogeo
        )
        ...
        if not checker.check_internet_connection():
            ...
            try:
                isogeo = Isogeo(
                    client_id=self.tab_options.isog_app_id.get(),
    ...
    def change_lang(self, event):
        ...
        TextsManager().load_texts(
            dico_texts=self.blabla, lang=new_lang, locale_folder=r"data/locale"
        )
        # update widgets text
    ...
    def process_files(self):
        ...
        if self.tab_files.opt_pdf.get() and len(self.li_pdf) > 0:
            ...
            for pdf in self.li_pdf:
                ...
                try:
                    ReadGeoPDF(
                        path.abspath(pdf), self.dico_pdf, "Geospatial PDF", self.blabla
                    )
                    logger.info("\t Infos OK")
                except (AttributeError, RuntimeError, Exception) as e:
                    """ empty files """
        ...
        if self.tab_files.opt_lyr.get() and len(self.li_lyr) > 0:
            ...
            for lyr in self.li_lyr:
                ...
                # getting the informations
                try:
                    ReadLYR(path.abspath(lyr), self.dico_layer, "Esri LYR", self.blabla)
                    logger.info("\t Infos OK")
                except (AttributeError, RuntimeError, Exception) as e:
                    """ empty files """
                    logger.error(e)
    ...
    def process_isogeo(self):
        ...
        for md in dico_md.get("resources"):
            ...
            tags = md.get("tags")
            li_motscles = [
                tags.get(tag) for tag in tags.keys() if tag.startswith("keyword:isogeo")
            ]
            li_theminspire = [
                tags.get(tag)
            ...
            link_visu = 'HYPERLINK("{0}"; "{1}")'.format(
                url_oc + "/m/" + md.get("_id"), "Visualiser"
            )
            link_edit = 'HYPERLINK("{0}"; "{1}")'.format(
